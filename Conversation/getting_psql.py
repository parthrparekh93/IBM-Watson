...
#get time of oh
s = "SELECT pname, oh_time FROM professor2 JOIN course_prof2 ON professor2.pid = course_prof2.pid where\
  to_tsvector('english', pname) @@ to_tsquery('english', '" + prof + "')"
print doQuery(s)
...
s = "SELECT name, cno, lec_time FROM course JOIN course_prof2 ON course.cid = course_prof2.cid where\
  to_tsvector('english', name) @@ to_tsquery('english', '" + course + "')"
print doQuery(s)
...
print doQuery("SELECT course.cno, course.name, loc_no, loc_code, building.x_co, building.y_co FROM\
course_prof2 JOIN building ON course_prof2.loc_code = building.b_code JOIN course ON course.cid = course_prof2.cid\
where to_tsvector('english', course.name) @@ to_tsquery('english', '" + entity_value + "')")
 
# Intent = reviews, entity = professor_or_course, value = professor
entity_value = "bellovin"
print doQuery("SELECT pname, review, sentiment FROM\
professor2 JOIN course_review ON professor2.pid=course_review.pid where to_tsvector('english', pname) @@ to_tsquery('english', '" + entity_value + "')")
...
print doQuery("SELECT name, sentiment, review FROM\
course JOIN course_review ON course.cid=course_review.cid where to_tsvector('english', name) @@ to_tsquery('english', '" + entity_value + "')")
